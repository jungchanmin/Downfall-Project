...
intro_narration: |
  ...
  지도를 살피던 리더가 인기척을 느끼고, 턱을 천천히
  들어 문턱에 멈춰 선 레이첼을 똑바로 응시했다.
  두 사람의 시선이 마주치는 순간, 레이첼이 무언가를 말하려는 듯 입술을 조금 벌렸다. 
  그러나 그녀의 목에선 짧은 호흡만 가쁘게 맴돌 뿐, 그녀의 어떤 말도 입밖으로 새어 나오지 못했다. 
  ...
  그녀는 리더가 다시 시선을 돌릴때까지, 아무 말도 꺼낼 수 없었다. 
  레이첼은 입술을 짓이기며, 검자루를 세게 틀어쥐었다. 
  분명히 하려고 했던 수많은 말들이 그녀의 가슴에서 맴돌뿐, 입으로는 튀어나오지 않았다.
  한없는 정적 가운데 가끔 삐걱거리는 소리만 두 사람의 사이로 맴돌았다.
  ...
  - id: ch1
    ...
    resolution:
      - condition: { stat: { actor: "Rachel",
          친밀도: { lt: 3 } } }
        narration: |
          레이첼은 결국 말을 거는 것을 포기했다. 
          그녀는 단 한 마디도 하지 못한채로, 그녀는 고개를 아주 약간 아래로 숙여 인사하고는 자리를 떴다. 
          리더는 발길을 돌려 멀어지는 그녀에게 말을 걸거나 불러세우지 않았고, 잠깐 시선을 던진 뒤 이내 하던 작업으로 돌아갔다. 
          지도가 바스락거리는 소리가 거실을 채우는 동안,
      ...
      - condition: { stat: { actor: "Rachel",
          친밀도: { gte: 3 } } }
        narration: |
          레이첼은 침묵을 유지한채로, 리더의 앞에 펼쳐진 종이지도를 향해 시선을 던졌다. 
          그녀는 애절한 눈빛으로, 리더가 다시 자신을 바라봐 주기를 기다렸다. 
          그러나 리더의 시선은 이제 그녀에게서 멀어져 묵묵히 지도 위에 표시해둔 검은 표시선만을 좇고 있을 뿐이었다.
          두 사람의 시선이 철저하게 엇갈리는 정적 속에서 레이첼은 오른손으로 검자루를 한 번 더 굳게 움켜쥐었다.

          그녀가 마른침을 삼키고 간신히 숨을 밀어내면서, 조각난 단어 하나를 그녀의 입술사이로 힘겹게 내뱉었다.
          ...
          레이첼은 더 이상 대답하지 않았다. 
          그럴 필요가 없었다.
          굳게 다문 입술과 긴장한 표정사이에 서서히 퍼지는 희미한 미소는 그녀의 복종의 뜻이자 애원이나 다름 없었다. 
