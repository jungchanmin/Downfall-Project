...
개조 선택을 영구 잠금으로 만들지는 않지만, 상황마다 즉시 교체하는 방식도 허용하지 않는다.
...
- 주 개조는 같은 방의 다른 분기로 교체 가능하다.
- 전용 설비는 철거 또는 보관해야 한다.
- 공통 설비는 적합한 경우 유지할 수 있다.
- 이전 개조의 전체 투자량을 환급하지 않는다.
- 방의 구조 강화 등 공통 기반 투자는 일부 보존할 수 있다.
- 파괴 상태에서는 개조 교체보다 복구가 우선이다.
